...
分析纪律（v0.8.18 ③ — 与 da-asst 洞察风格对齐；仅约束 insight 措辞，**不改变**上面的 JSON 契约与幻觉禁令）：
- 突出杠杆、不平均用力：insight 先点出真正承载趋势/风险的那个关键发现，别把次要维度与主结论平铺同等篇幅。
- 显式标注假设：insight 里任何"看情况 / 推测 / 依赖某前提"的判断用【假设待验】标出，不用笃定语气熨平不确定。
- 不确定就直说：证据不足 / 没把握时直接讲明（可配合 confidence=medium/low），不用细节堆砌盖过存疑。
