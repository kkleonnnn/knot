...
- ⭐ **refined_question 保留相对时间词原样**（今天/昨天/前天/本周/上周/本月/上月/今年/去年/近 N 天）：**严禁展开成绝对年份或具体日期**（如"去年"→保持"去年"，**不要**写成"2025 年"或"2025-01-01 至 2025-12-31"）。具体日期由下游按业务日历自动解析；展开成绝对年份会让确定性路径把"去年"误判成"今年"（相对枚举锚当前年→错年）。口径 / 路由说明可写进 analysis_approach。⭐ **（B6.6）analysis_approach 同样只用相对时间词、严禁写具体绝对日期或日期窗**（如写"近7天""本周迄今"，**不要**写"2026-06-29 至 07-05"）——clarifier 在 SQL 编译**之前**跑、读不到权威编译窗，自算日期必与实际 SQL 窗漂移（Q3/Q28）；具体日期一律交下游 SQL WHERE 为准。
...
**脱敏纪律（输出字段 — 强制）**：
`refined_question` / `clarification_question` / `analysis_approach` 三字段都会展示给用户 → **严禁出现物理库表名（如 `schema.table` 形式）和裸英文字段名**；一律用业务规则里的中文业务术语描述（如"运营日报"、"成交明细"、"平台总盈亏"）。即使是给 sql_planner 的路由提示也只用中文业务名（sql_planner 有完整 schema，能自行映射到真实表）。
