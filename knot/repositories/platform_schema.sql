...
-- ⚠️ **不变量**：`db_dir` 建成后**永不重写**（改 slug 不搬数据）。
--   ✅ **已强制**（v0.9.15 d4 落地）：`db_dir` 不在 `_MUTABLE_TENANT_FIELDS` 里
--   ⇒ 走写口改它抛 `ValueError`；守护 `tests/test_platform_audit.py::test_d4_db_dir_is_no_longer_mutable`
--     （断言「被拒」**且**「值真的没变」）。
--   ⚠️ **本注释曾长期停留在「截至本注释写下时它还没被强制…在那之前这只是一句意图」**
--     —— 而 d4 早已落地。v0.9.20 的一次独立评审**信了这句话**，据此报了一条**并不存在**的缺陷。
--     ⇒ **过期注释不是文档瑕疵，它在主动制造假发现**：判据要锚在系统真的产出了什么，
--       而不是某个文件里写着什么。改了守护，就要回来改声称它不存在的地方。
