...
def _validate_hit(lf: LogicForm, metrics: list[dict]) -> bool:
    ...
    v0.7.11：放宽多对象拒绝 —— 允许**标量多 base**（无维度，compiler 标量子查询入 SELECT）；
    多 base + 维度 → 仍拒（parser pre-guard；compiler 再全矩阵 raise-guard 双防）。
    单对象 + 跨对象维度照常（v0.7.2）。
    """
    if not lf.metrics:
        return False
    ...
    if "" in objs:
        return False
    if len(objs) > 1 and lf.dimensions:   # 多 base + 维度 → 回退（聚合后 JOIN 留后续刀 R-SL-101）
        return False
    return True
