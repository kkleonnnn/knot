...
### chore（riding v0.9.15 **不 bump**）—— `tests/` + `scripts/` 纳入 ruff 闸门（先清债再开闸）

**为什么这是真缺口，不是整洁问题**：CI 此前只跑 `ruff check knot/` ⇒ `tests/` 不受任何 lint 约束，
实测积累 **73 处 / 43 文件**。而其中**两类不是噪声**：
- **`F841`（赋了值没用）**：在**测试**里往往意味着「**算出来了却没断言**」；
- **`B023`（闭包未绑循环变量）**：晚绑定 bug 类。

⭐ **实证 —— `F841` 真逮到一处「看起来测了而没测」**：`tests/services/test_schema_filter.py` 有两行死代码
`out = f("有哪些表", text, …) if False else f(text, "…", …)` —— `if False` 让第一个调用
（**参数还是反的**）永不执行，而 `out` **从未被断言** ⇒ 那个测**看起来**覆盖了小文本那一路、实际没有。
已删（零覆盖损失，真断言在下面的 `out2`），并在原处留注释说明删了什么。
⇒ **给测试加 lint 闸门会暴露「看起来测了而没测」。**

**73 → 0 的处置分四类，逐条核过**（不用 `--unsafe-fixes` —— 它对 `F841` 的「修法」会把
`a = create_few_shot(...)` **整句删掉**，而那个调用是测试前提 ⇒ 会静默削掉覆盖）：
① 机械修 43（I001 27 自动 · E702 14 拆分号 · UP045/PLW0108 各 1）·
② 去绑定保调用 4（F841，调用有副作用）· ③ **真发现删掉** 2（上面那条）·
④ **判为误报 ⇒ noqa + 写明失效条件** 12（B023 3 行：闭包在**同一次迭代内**就被 `gather` await 掉
⇒ 晚绑定结构上不可能；**并写明「若 gather 移出循环，这三条豁免立刻失效，届时须改默认参数绑定」**）。

**开闸顺序刻意**：先清到 0 再开闸 —— 否则第一次红淹在存量噪声里，分不出哪条是新问题
（哨兵上线即一片红 = 等于没上线）。`scripts/` 一并纳入：实测**本就 0 处** ⇒ 零代价。

**扫两侧（v3.1-B #8）**：全仓 **33 个文件**提到旧命令 —— 只改 `CONTRIBUTING.md`（**活文档**，
贡献者照着跑），`CHANGELOG` 与 `docs/plans/*` 是**历史记录，不动**（当时确实只扫 `knot/`）。
新增**双侧派生**不变量 `test_contributing_ruff_command_matches_ci`：两边都从文件读、不写死期望值
⇒ 下次改 CI 扫描面不必改测。取材：把 CONTRIBUTING 改回旧命令 → 唯一抓住，
失败消息把两边原文都打出来。

⚠️ **实施期我犯的三个错（第 2 条最值得留）**：
① 脚本把 5 个文件改出**语法错**（「扫括号平衡找收尾」从行内其他括号开始计数就错）。
② ⭐⭐ **第 6 处更险：`check=False` 被插进了 `.strip()`** —— **语法合法**，`ast.parse` **通过**，
只有**真跑测试**才抓到（`TypeError`）。⇒ 随后加 AST 复核：**每个 `check=` 都必须挂在
`subprocess.run` 上**（实测 0 处挂错）。**「语法过了」与「作用在对的对象上」是两个不同的判据。**
③ 新哨兵里常量名写错（`REPO` 该文件不存在）⇒ 测从一开始就红在 `NameError`
⇒ 那次「取材成功」是假的，读失败消息原文才发现。
...
- **`reset_admin_password` 强制 `--tenant`**：该脚本此前**完全没有参数解析** ⇒ 有人按本版开通端点
  的 409 指引执行 `… --tenant <slug>`，flag 被**静默吞掉**，脚本照常重置
  「唯一 active 租户」（= 起源租户 / 部署方自己）的 admin 口令，并打印「已重置」。
  **运维看不出任何异常。**
  ⭐ **这不是推理 —— 它真的发生过**（执行者为验证该问题而在真实库上执行）。
  ⚠️ 本记录**只写事件不写值**（不含任何凭据片段）。
  ⇒ 修法是**强制**：`--tenant` 必填、缺省即拒（非 0 退出 + 零写入）· 写之前先打印目标
  （id + slug + status）· 走不过滤 status 的 `get_tenant`（正当用途之一就是给刚开通、
  仍 `suspended` 的租户重置）· 4 条测含 AST 静态钉住 `required=True`。
  ⛔ **严禁改回「可选 + 回退唯一 active」** —— 同一事故会原样复发，而那个回退形态
  正是 v0.9.4 记过的坑（登录代号可选 ⇒ 放开多租户后 fail-open）。**破坏性工具不得有默认目标。**
- **slug 格式校验**（仅小写 `^[a-z0-9][a-z0-9-]{1,30}$`）：决定性理由是**大小写** ——
  数据库唯一性检查**区分**大小写 ⇒ `Acme` 与 `acme` 是两个租户，而登录链接肉眼**完全一样**
  （混淆/钓鱼面）。⚠️ 这一半与「不透明目录名」关掉的那一半是**两个**洞，
  一条论证只关得掉一个。正则做成**单一真相源**，API 与写口两个执行点共用。
  ⚠️ **守它的那条测初版判据是错的（同轮一并修）**：写成 `pa._SLUG_PATTERN is …` ⇒ 锚在**名字**上
  ⇒ 别名一改它就红而正则**依然是一份**；且 `SLUG_PATTERN in pats` 是**相等**比较
  ⇒ **抄一份逐字相同的字面照样绿** = 对要防的失败模式零判别力。改 **AST 判「引用还是字面」**
  （`ast.Constant` 即红）+ 行为判等。取材：抄逐字相同的字面 → `1 failed, 17 passed`。
  ⚠️ 副作用：`platform_admin.py` 由此贴到 size gate 顶（300/300，登记 `BL-v0915-2`）——
  `Field(pattern=…)` 在导入期求值 ⇒ 必须顶层 import ⇒ 端点内那处延迟 import 成死重故删。
- **路径逃逸必须留痕**：fail-closed 顺序本就正确（拒绝发生在建目录之前），但请求路径会把它
  折成一条普通 401 并丢弃原消息 ⇒ 一次**文件边界违规**与「租户停用」**无法区分**且全程零日志。
  ⇒ 在**抛处**记 error（那才是事情真的发生的那一行），内容 = `tenant_id` + `db_dir`；
  ⛔ **不记解析路径与数据根** —— 它们由 `SQLITE_DB_PATH` 派生 = env 派生值，进日志即 #262 家族。
  ⚠️⚠️ **本条的初版是个「源码看着对、产出是空壳」的东西，值得单独记**：写成 stdlib 形式的
  `logger.error(msg, extra={...})`，而 `logger` 是 **loguru** —— kwargs **只喂 `str.format()`**，
  消息无占位符 ⇒ **整个 dict 静默丢弃**，实测输出仅 `'tenant_db_path_escape'`
  ⇒ 留痕有了而「**是谁**」没了。**这是本弧「消息说的对吗」的新形态**：
  v0.9.10 是「显示了但内容是假的」，这次是「**显示了而什么也没说**」——
  两者都只有看产出才发现。⇒ 判据必须是**产出**：`test_iso2c` 用 loguru sink 断言
  `tenant_id=` 与 `db_dir` 值真的在输出里（**结构型哨兵会祝福那个空壳** ——
  实测同一版下「源码里 `logger.error` 调用数 = 1」，任何「断言存在该调用」的判据都通过）。
  两次取材各自**唯一抓住**（`1 failed, 21 passed`）。

### ⛔ 第 5 条（执行者自查）—— 同一条规则的另外两个实例，**都是本片造出来的**

守护者 §III：「本片那道网守的是**测试路径**，事故从**脚本路径**进来 —— 只堵了一条。」
⇒ 据此问「**还有哪些路能到达同一个后果**」，答案是**还有两条**。
**因果**：本片之前全仓只可能存在一个租户 ⇒ `resolve_single_tenant()` 回退无处可错；
本片起新租户开成 **`suspended`** ⇒ 回退**恒解析到起源租户**，而运维想的是那个新租户
⇒ 动作静默作用在错误的库上、输出照样报「完成」= 与 #1 逐字同形。

- **`migrate_encrypt_v045`**（处理**凭据列**）：目标互斥组改 `required=True`
  （`--tenant` / `--all-tenants` 二选一），隐式回退**物理删除**。DEPLOY 范例本就带 `--tenant 1` ⇒ 文档零改动。
- **`purge_audit_log`**（删**审计行**）：**真跑必填** `--tenant`；`--dry-run` **仍可省**
  （0 副作用，且新增「打印解析到了谁」）。⚠️ **刻意不一刀切** —— DEPLOY 的诊断范例正是
  不带目标的 `--dry-run`，一刀切会破运维手册；规则管的是**破坏性动作**。
- **`scan_secrets_at_rest`** 不改（v0.9.12 刻意只读 ⇒ 回退最坏是扫错一家，无损害）。
- **分类哨兵**：`knot/scripts/` 里凡调用 `resolve_single_tenant` 的都必须在**表态清单**里写下理由
  （AST 判被调名 —— 文本匹配在这里自匹配：多个文件正**讨论**这个名字）。
  配反向条：清单里不得留**已不需要**的条目（否则清单会祝福不存在的风险）。
- **oracle 避坑（第 ③ 种失效「判据恒定」）**：不能用「audit_log 行数没变」证明零写入 ——
  retention 只删 N 天前的行，新 tmp 库里没有够老的行 ⇒ 真跑也删 0 行 ⇒ **恒真、零判别力**。
  改用 `_make_backup()` 的产物（真跑无条件先建 bak）+ 反向守护证明它非空判据。
- **第 6 条守护**：两脚本必须能作用于**刚开通的 `suspended` 租户** —— 本片让新租户恒 suspended，
  而运维最先要做的两件事（补迁移旧凭据 / 按合规清审计）都在它 active 之前 ⇒ 走 `resolve_*` 则**结构上到不了**。
  仓里的 `get_*`/`resolve_*` 命名约定是**散文**，改错了不会红；这条会。
- **六次取材逐条唯一抓住**：摘 purge 门 `1/7` · 迁移回退 **`3 failed`**（行为 + AST + 分类三层同响）·
  未表态脚本 `1/7` · 过期清单条目 `1/7` · 文档漂移 `1/8` · 解析换 active-only `1/9`。
  ⚠️ **最后那次的第一版取材是假的**：注入了一个**不存在的函数名** ⇒ 红在 `AttributeError` 而非语义，
  且假取材红 **2** 条、真取材只红 **1** 条 ⇒ 判别力完全不同。
  ⭐ 第 ② 问的干净实例：**「它红了」≠「它因为我要测的原因红了」** —— 判据是读失败消息原文，不是看颜色。

⭐⭐ **最后那次取材抓的是我自己刚犯的漏**：修 #1 时我改了脚本、写了测、写了 CHANGELOG，
**却漏了「描述它的地方」** —— `DEPLOY.md` 两处 + `README.md` 一处的范例都不带 `--tenant`
⇒ **那三条命令在修完的那一刻起就照抄必被拒，而四闸门全绿。**
⇒ 兑现 v3.1-B #8「**扫两侧：描述它的地方，和断言它的地方**」，并装成哨兵：
活文档里的调用范例必须真能跑。**扫描面派生**（`git ls-files '*.md'` 减 `docs/plans/`）
而非枚举白名单 —— 后者会漂（新活文档没人记得加）；实测切换**零代价**，命中集相同而扫描面 3 → **26** 个文件。
配两条边界取材：**空扫描面必须响亮地红**（第 ③ 问在扫描型哨兵上的形态 ——
扫描面空掉的哨兵永远绿，而它已经什么都不守了）· 往 `docs/plans/` 塞陈旧命令**刻意不红**
（那是「当时是这样」的留痕，改它等于篡改历史 ⇒ 排除项是有意的**且可证**）。
新增 DEPLOY「破坏性 CLI 一律要显式 `--tenant`」段（含四脚本对照表）。
⭐ **规则已折进 CLAUDE.md v3.1-B #1（fail-open 面）作具名子形态**（kk 2026-08-05 拍板）——
**刻意不新增第 12 条**：守护者说过表膨胀会降低逐条回答的质量。
折入时补了一句本片才看清的话：**「可选 + 回退唯一/默认对象」在对象集合从 1 变成 N 的那一刻起就是 fail-open** ——
它在只有一个对象的年代无处可错，所以看起来一直是对的。
...
⭐ **守护已做**（kk 2026-08-03 拍板「加」）：`test_doc_invariants.test_readme_previous_version_link_is_not_broken`
—— 判据**从 CHANGELOG 派生**（那是版本序列的真相源），不写死期望值：README 的「上版」
必须等于 CHANGELOG 里**紧挨当前版本的前一条**；且「上版」标记**恰 1 个**。
两条合起来闭合「漏插自己那条」的两种形态（压根没加 / 加了新的却忘把旧的降级）。
⭐ **revert-to-bad = 唯一抓住**：把「上版」改回 v0.9.9（**历史上那个真实的断链状态**）⇒
`1 failed, 18 passed` —— 另外 18 条版本相关断言**结构上看不见叙述链断裂**，
这正是四片能静默缺席的原因。失败消息直接给出该写什么：
「本片漏插了自己那一条：把现在的「上版」改成 `**更早**`，并在它前面插入 `<br>**上版** v0.9.13 · …`」。
...
> ⚠️ **测覆盖口径校正（守护者 Stage 4 §III）**：本 PATCH 中 **R2 回归守 / 归一化列签名 / 孤儿 marker / F1 / F2 / F3 / 探针强度 / 端到端 join 均经 revert-to-bad 实证**（改回坏行为 → 对应测确实转红）；但 **MF2 的「两 commit-per-table」WAL 跨附加库原子性修属 correct-but-untested** —— 单测抓不到跨库 crash（需故障注入），生产码经三方 grounded 复核为对，但**不得当作 test-backed**。
...
### Fixed（守护者 Stage 4 + 执行者第二轮对抗）
- **skip:relocated 补 C4-parity 健康探针（Stage 4 should-fix）**：relocation 完成后 target 若被掉电/WAL-replay 损坏，原仅凭 `target.exists()` 返 `skip:relocated` → 建引擎于损坏库、运行时静默失败无 halt（C4 姊妹 `_migrate_locked` #5 是 enforce 的，此处曾不对称）。新 `_uploads_unhealthy_reason`（`PRAGMA quick_check`，**0 表仍合法** —— 不复用 C4「零表即 False」的 `_db_wellformed`）→ 不健康即 fail-closed halt。
- **F1 · boot crash-loop 修（三方独立命中）**：resume 分支保全既有 target 时，`_uploads_has_real_data` 读不出即返 True（保守），于是**半写/损坏 target 也会走保全** → sqlite `.backup()` 读不出它 → `DatabaseError` 裸逃出 `main.py:90`（无 try/except）→ **永久 boot crash-loop**（C4 姊妹在同态下自愈）。改为整字节 `os.replace` 移存留证 + 由仍在的 src 重做 relocation 自愈。
- **F2 · 恢复指引修（原指引会加剧故障）**：原文案让运维「从 data-root 的 `.bak` 还原」→ 下一轮启动撞「源与 target 并存」安全阀二次 halt；且该 `.bak` 在 skip:fresh 路径下**根本不存在**。且原文案直接建议「删除损坏文件」—— 而锁竞争/权限（`OperationalError`）与真损坏在探针里同形，据此删文件是**数据丢失向量**。新文案：先按「重启是否复现」判性质（锁/权限所致重启即恢复）→ 还原须还原到 **target 本身**（`若存在` 才提）→ 删除仅作最后手段。探针改为返回**原因原文**（含 sqlite 报错）供分辨。控制流故意不区分锁与损坏（narrow 到 `DatabaseError` 只会让 `OperationalError` 裸 traceback 逃出，更糟；实测 4 并发上传/1993 MiB 库/33 次启动 0 误判，最坏 1.8s « 5s busy timeout）。
...
> **⑤ 冻结公式行不透明修（kk 逐图·二轮①）**：sticky 公式行 bg 原用 accentSoft（**dark 是 0.14 alpha 半透明** → 滚动数据透视穿过）。改 `linear-gradient(accentSoft,accentSoft), content`（半透明叠实底 → 合成不透明同色调，两模式都不透）。kk 二轮②（重跑重置插入行）**经查非 bug**：`update_tile_last_run` 只写 last_run_*、不碰 viz_config；实测 refresh 运营日报 overlay byte-identical 存活（②c 调度器亦安全）；kk 见的消失 = 我清了财务日报 stray 合计（巧合早于其重跑）。
> **⑥ tabbed 报表导出（kk 逐图·二轮③）**：放开多页表导出（dashboard 仍拒=图表非表格）。**Excel=多 sheet**（每页一 sheet，新 `sheets_to_xlsx_bytes`，镜像 Google Sheet）· **CSV=当前页**（`export.csv?tile_id=`；TabbedTableReport `onActiveTile` 上报当前页给 BI.jsx）；均 **中文 label 表头**（export_service +`headers` 参；`_neutralize_text` 注入中性化沿用）。BI.jsx 导出按钮 wide_table→+tabbed。
...
> **闸门**：后端 pytest（+overlay 超限→400 + 非 dict viz_config 不 500 + tabbed 导出 CSV 当前页中文表头/Excel 多 sheet + dashboard 导出仍 400）· vitest 37（+列字母/除零/非数组）· eslint · ruff · R-94（+OverlayEditor 登记 + bi_reports ACK 360）· doc-invariant（版本 4 源点 v0.8.9）· R-BI-1 ASK 0 触。
...
>
> **后端**：新 `bi_report_tiles` 子表（report_id soft ref / tile_type / sql_text / viz_config / sort_order / grid_span / per-tile last_run_* + last_run_error；无 per-tile refresh_seq — D2 整表原子）· `bi_report_tile_repo` · service **diff-by-id** 同步（**B-1** id 归属校验防越权改他表 tile / 保未改 tile 快照 / 每 tile SQL `is_safe_sql` / `_MAX_TILES=30`）· **B-2 脱敏** tiles 组装放 getter + `to_dto` 非-admin 逐 tile 深拷后 pop sql_text（两路径 guard，镜像 v0.7.27）· refresh dashboard 分支循环 tile 各自快照 + report 级 `touch_refresh_seq` bump-only（**B-3**）· 路由 `run_in_executor` 卸载 + 审计 detail 按 report_type 分支（**B-6**）· 导出闸 dashboard→400（**B-7**）。审计计数 58 不变（复用 bi_report.update/refresh）。
> **前端**：DashboardReport 读 tiles[] 分发 5 tile 渲染器（KPI valueCol 显式 **C-3** 吸 v0.7.30 ID-like 列坑 / Line·Shared / Donut·自绘 / Bar·横条 / Table·复用 orderedCols+fmtValue）· `TileBuilder`（type/SQL/viz 映射/占列 + 手写 HTML5 DnD reorder，R-186 无库）· ReportBuilderModal 双类型接入。**size-gate 补 bi/ 全登记**（critic D 盲区闭合）。
> **闸门**：后端 pytest（tile repo/service/api + per-tile 脱敏/归属/DoS guard）· eslint · vitest · doc-invariant（KnotLogo 4 / 版本 4 源点 v0.8.6）· R-94 · R-BI-1 ASK byte-equal 0 触。

## [Released] - v0.8.5 — ②a BI 报表模式（问数/报表双模第一刀）

> v0.8 ②（报表双模）第一刀。问数(ASK)/报表(BI)双模切换 + **宽表**报表（admin 直写 SQL + 覆盖层 Excel 式公式）。走完整 v3；守护者 Stage 3 = ACCEPT WITH REVISIONS（approve-with-conditions，逐条整合）。plan [`docs/plans/v0.8.5-bi-mode-shell-widetable.md`](docs/plans/v0.8.5-bi-mode-shell-widetable.md)。
> **⭐ 前端架构（kk live review 根因修）**：BI 与 ASK **共用同一 `<AppShell>`**（侧栏壳 + brand + topbar + 用户 footer 统一，只切侧栏中段 + 主区两 slot）+ inline 分段 `ModeToggle` + 共享 `RightPanel`（BI 数据分析 ∥ ASK 思考过程）+ 子屏返回来时模式；覆盖初版「独立 BIShell + 悬浮 toggle」。10 项逐图打磨 + 2 轮对抗复核。
>
...
> **前端**：BIScreen（走共用 AppShell）+ ReportDirectory（文件夹树）+ WideTableReport（sheet/冻结列/条件色/覆盖层）+ DashboardReport（overview_grid）+ ReportBuilderModal（双类型 builder）+ SkillPanel（da-asst UI 壳，真接 ③）；BI 专属 icon inline SVG（0 触 Shared.jsx）。
