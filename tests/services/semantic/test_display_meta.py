"""v0.7.23/.25/.27 C1 — _semantic_display_meta 守护（呈现 meta：表头中文 #5 + 图表 dimension_cols 硬化 + 值格式 column_formats + 维度中文标签 field_labels）。
...
- R-SL-187/188 (v0.7.27) field_labels 维度中文：仅补 lf.dimensions 列 + 仅 d ∉ column_labels（metric 优先）；空 field_labels → byte-equal
"""
...
# ─── v0.7.27 维度中文标签 field_labels merge（R-SL-187/188）────────────────────

def test_dimension_label_merge():
    # field_labels[d] merge 进 column_labels（仅 lf.dimensions 列）；metric 走 display，维度走 field_labels
    labels, dim_cols, _ = _semantic_display_meta(
        _lf(["user_position_pnl"], ["market", "sta_date"]),
        {"user_position_pnl": {"display": "持仓盈亏"}},
        {"market": "交易对", "sta_date": "日期", "unused_col": "无关"},  # unused_col ∉ dimensions → 不入
    )
    assert labels == {"user_position_pnl": "持仓盈亏", "market": "交易对", "sta_date": "日期"}
    assert dim_cols == ["market", "sta_date"]


def test_field_labels_only_dimensions_not_all_keys():
    # R-SL-188：仅补 lf.dimensions 列，严禁遍历 field_labels 全 key（防污染非结果列）
    labels, _, _ = _semantic_display_meta(
        _lf(["gmv"], ["market"]),
        {"gmv": {"display": "成交额"}},
        {"market": "交易对", "other_col": "其它", "yet_another": "又一个"},
    )
    assert labels == {"gmv": "成交额", "market": "交易对"}  # other_col/yet_another 不入


def test_field_labels_metric_priority_no_overwrite():
    # R-SL-188 承重：极端误配 dim 名 == metric 名 → metric.display 优先，field_labels 不覆盖
    labels, _, _ = _semantic_display_meta(
        _lf(["clash"], ["clash"]),  # clash 既是 metric 又误列为 dimension
        {"clash": {"display": "指标口径"}},
        {"clash": "维度标签"},
    )
    assert labels == {"clash": "指标口径"}  # metric.display 优先（d 已在 column_labels → 不覆盖）


def test_empty_field_labels_byte_equal():
    # R-SL-187：field_labels 空 → column_labels 仅 metric（v0.7.25 byte-equal，additive-only）
    labels, dim_cols, formats = _semantic_display_meta(
        _lf(["gmv"], ["market"]), {"gmv": {"display": "GMV"}}, {},
    )
    assert labels == {"gmv": "GMV"}  # 维度 market 无标签 → 不入（前端 fallback raw {c||c}）
    assert dim_cols == ["market"]
    assert formats == {}
