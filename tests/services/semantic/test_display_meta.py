"""v0.7.23 C1 — _semantic_display_meta 守护（呈现 meta：表头中文 #5 + 图表 dimension_cols 硬化）。
...
def test_column_labels_metric_display():
    labels, dim_cols = _semantic_display_meta(
...
def test_empty_display_fallback_to_name():
    labels, _ = _semantic_display_meta(_lf(["gmv"], []), {"gmv": {"display": ""}})
    assert labels == {"gmv": "gmv"}  # 缺 display → fallback name（R-SL-169）


def test_metric_not_in_by_name_skipped():
    labels, _ = _semantic_display_meta(_lf(["gmv", "unknown"], []), {"gmv": {"display": "成交额"}})
...
def test_dimension_cols_is_lf_dimensions_not_metrics():
    ...
    _, dim_cols = _semantic_display_meta(_lf(["gmv"], ["date"]), {"gmv": {"display": "GMV"}})
...
def test_no_dimensions_empty_list():
    ...
    _, dim_cols = _semantic_display_meta(_lf(["gmv"], []), {"gmv": {"display": "GMV"}})
    assert dim_cols == []
