...
R-72：FastAPI title=KNOT version=<current>；routes count 下限 >=80 守护（v0.6.0.13 起改下限；实际 flatten=144 @ v0.8.21）
...
def test_R72_app_title_and_version():
    """FastAPI title=KNOT version=0.8.21（v0.5.0 R-72 守护，每 PATCH 同步）。"""
    from knot.main import app
    assert app.title == "KNOT", f"title 应改 KNOT；实际：{app.title}"
    assert app.version == "0.8.21", f"version 应 0.8.21；实际：{app.version}"
