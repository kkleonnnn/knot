...
_DOMAINS = ["users", "datasources", "models", "api_keys", "budgets", "stats", "or_catalog", "metrics", "logicform", "monitors", "share"]


def test_admin_router_aggregates_53_routes():
    """R-AS-1：admin.router 聚合 11 域 = 53 路由（穿透 FastAPI 0.137 懒包装）。

    47（41 + monitors 6）+ v0.8.14 share 5 = 52；+ v0.8.21 datasources/status 1 = 53。"""
    ...
    #    横跨 **5 个模块**（admin 包 + catalog + audit + feedback + frontend_errors）——
    #    只改 admin 包会漏 37 条（14 前缀内包外 + 23 前缀外）。
    assert len(admin_routes) == 53, f"admin 应聚合 53 路由（52 + v0.8.21 datasources/status）；实际 {len(admin_routes)}"
