...
## §7 v0.6.2.2 R-LP-v3-EX-1 扩展适用先例（2026-06-08 记录）

### 跳 Stage 2 资深裁量 — R-LP-v3-EX-1【扩展适用】

**背景**：v0.6.2.2 复合 metric 修复 PATCH，ζ1 原决定"补做 Stage 2" → 资深 2026-05-29 反转跳过。

**与 R-LP-v3-EX-1 原定义的差异**：
- R-LP-v3-EX-1 原定义：方向决策 / docs / ci / chore（**0 业务代码 + 0 红线新立**）
- v0.6.2.2 实际：**含业务代码**（MetricCard 多值网格）+ **新立红线**（R-PB-A5-1/2）
- 故属 R-LP-v3-EX-1 **扩展适用**，非原定义命中

**扩展可接受的三条件**（v0.6.2.2 全满足 — 守护者第 22 次 §V.B 裁定）：
1. 守护者草案评（第 21 次）深度覆盖 — 含 fan-out 守护粒度核心判断 + 4 整合修订全清偿
2. 范围窄单 bug — 复合 metric 前端展示 + prompt 指引
3. 红线复用而非全新机制 — R-PB-A5-2 复用既有 v0.5.1 fan-out AST（非新硬守护）

**边界声明（防"含业务代码就跳 Stage 2"滑坡）**：
- 本扩展适用**不成为默认** — 未来"含业务代码 + 新立红线"PATCH 默认仍走完整 Stage 2
- 仅当三条件全满足 + 资深裁量 + 守护者认可**联合决策**时方可跳
- 对比基线：v0.6.2.0 TOTP / v0.6.2.1 HTTP 路由（复杂业务 + 多新红线）仍走完整 Stage 2

**性质判定**：此跳过**不属于 OVERRIDE**（R-LP-v3-EX-1 简化协议扩展 + 三条件 + 联合决策）。
- §1 OVERRIDE 累计表**不变**（sustained 3 次）✓
- 守护者第 22 次 active explicit 认可（batch accept 4.8/5）+ 资深连续 announce 继续 ack

**记录依据**：守护者第 22 次 §V.B 议题 1（治理诚实度 — 记录先例 + 边界声明，commit 已 merge 不阻塞，记录后置）。

---

## §8 v0.8→v0.9 滚动整体审核 — OOS-1 翻转定性登记（2026-07-16 资深仲裁 A3）

**背景**：v0.8→v0.9 MINOR 滚动整体审核（三方独立意见 + 资深仲裁）。远古守护者（v0.6）主张「v0.9 启动 = 翻转 OOS-1 死线 = 第 4 次累计 OVERRIDE → 触发 R-LP-v3-EX-2.1」。执行者对抗验证（C2 PARTIAL）：**事实层成立**（全局累计 = 3 见 §1；http-adapter #1-#4 是 §2 独立子计数不入全局；v0.9 计划零预算此 gate），**但**「翻转红线 = OVERRIDE」是**条文未定义的治理解释** —— R-LP-v3-EX-2 / EX-2.1 均未定义「OVERRIDE」，更无「翻转红线 = OVERRIDE」字面；OOS-1 翻转走正式「红线修订仪式」，与维度 A「一个 PATCH 启动决定 = 1 OVERRIDE」性质有别。

**资深裁定（2026-07-16 · 整体审核决策 a = A3）**：
- **翻转 OOS-1 死线走正式「红线修订仪式」（执行者 + 守护者 + 远古守护者独立意见 + 资深仲裁）≠ 临时 OVERRIDE** → **不计入维度 A 全局 OVERRIDE 累计**。
- §1 OVERRIDE 累计表**不变（sustained 3 次）** ✓；R-LP-v3-EX-2.1（≥4）**未触发** ✓；**v0.9.0 无 Q-quarter 暂停前置**。

**消空白立约（防未来滑坡）**：今后「修订 / 翻转既有红线」凡经**完整红线修订仪式**完成的，**不计入全局 OVERRIDE**；仅**未走仪式、ad-hoc 逆转既有红线**才计维度 A。OOS-1 修订本身的治理严谨性由 v0.9.0 仪式 4 产物 + 独立 governance commit + 新隔离 CI 守护保证（整体审核决策 c/d）。

**性质判定**：此裁定**不属于 OVERRIDE**（红线修订仪式，非方向逆转 override），不计入 §1。

**关联立约**：整体审核决策 b → **R-LP-v3-EX-3-1**（default-admin 弱口令债正式红线，承诺序号 1）已入 `CLAUDE.md`。

---

**资深架构师签字**：2026-05-25（§1-§7）· **2026-07-16（§8 + R-LP-v3-EX-3-1 关联）**
**v0.7 守护者 / v0.6 远古守护者 ack**：v0.8→v0.9 整体审核三方齐备 + 资深仲裁 A3/B1/C1/D1 ✓
