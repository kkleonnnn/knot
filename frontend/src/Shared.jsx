...
export const I = {
  ...
  // v0.6.4.0 UI v2 additive +16（v0.7.48 死码清扫删 9 未用 → 存 7；viewBox 24 + stroke 1.6；路径源 knot_demo_ui/v0.6 ui.jsx）
...
// v0.6.2.3 commit 4 (R-PB-SH-6) — chip/badge 簇 7 single-copy 整合（byte-equal）：
// StatusDot(AdminAudit) NumChip/OverrideChip/SourceTag(tab_system) FileIcon(tab_knowledge) LetterChip/DoneCheck(ThinkingCard)
export function StatusDot({ T, ok }) {
  return (
...
export function OverrideChip({ T }) {
  return (
    <span style={{
...
export function SourceTag({ T, children }) {
  return (
    <span style={{
...
export function FileIcon({ T, type }) {
...
export function LetterChip({ T, letter }) {
  return (
    <span style={{
      width: 22, height: 22, borderRadius: 5,
...
// v0.6.2.3 commit 5 (R-PB-SH-6) — layout/style 簇 5 整合：FormRow(←Row AdminBudgets) /
// FilterField(←Field AdminAudit) / statCardStyle / statValueStyle / preStyle(AdminAudit)
// FormRow/FilterField body byte-equal，仅函数名改（R-PB-SH 泛名碰撞避免）；statValueStyle color:T.muted 怪癖保留
export function FormRow({ T, label, hint, children, last }) {
  return (
    <div style={{
...
export function statCardStyle(T) {
  return {
    background: T.card, border: `1px solid ${T.border}`,
