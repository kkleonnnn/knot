      ...
      {tab === 'sources' && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 16 }}>
            <div style={{ background: `color-mix(in oklch, ${T.accent} 8%, transparent)`, border: `1px solid color-mix(in oklch, ${T.accent} 25%, transparent)`, borderRadius: 10, padding: '14px 16px' }}>
              <div style={{ fontSize: 10.5, color: T.accent, fontFamily: T.mono, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 6 }}>已连接</div>
              <div style={{ fontSize: 22, fontWeight: 600, color: T.text, letterSpacing: '-0.01em' }}>{sources.length}</div>
            </div>
          ...
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr 1.4fr 0.6fr 0.8fr 80px', padding: '9px 16px', background: `color-mix(in oklch, ${T.accent} 8%, transparent)`, fontSize: 11, color: T.subtext, fontFamily: T.mono, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', borderBottom: `1px solid ${T.border}` }}>
            ...
            {sources.map((s, i) => (
              <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr 1.4fr 0.6fr 0.8fr 80px', padding: '11px 16px', borderBottom: i < sources.length - 1 ? `1px solid ${T.borderSoft}` : 'none', alignItems: 'center', fontSize: 12.5 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, overflow: 'hidden' }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: `color-mix(in oklch, ${T.accent} 8%, transparent)`, color: T.accent, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
