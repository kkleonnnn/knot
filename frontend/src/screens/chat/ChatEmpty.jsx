...
// v0.6.4.3 UI v2 — suggestion flex-wrap→grid auto-fit + chip T.text + icon 偿还（inline Spark/Flow → I.spark/I.flow，stroke 1.8→1.6 设计系统归一化）+ greeting gap 节奏
...
export function ChatEmpty({ T, user, question, setQuestion, loading, onSubmit, onKeyDown,
  ...
  // 业务 suggestion 文案（KNOT 真实域 — 合约/平台盈亏；非 demo GMV/订单 fiction）
  const suggestions = [
    { icon: <I.spark/>, text: '今天的合约交易总量是多少？' },
    { icon: <I.flow/>,  text: '最近 7 天每日平台总盈亏趋势' },
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 720, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
        <div style={{
        ...
        <p style={{
          fontSize: 14, color: T.subtext, margin: '4px 0 0',
        ...
        <div style={{
          ...
          {suggestions.map((s, i) => (
            <button key={i} onClick={() => setQuestion(s.text)} style={{
              minWidth: 0, height: 44, padding: '0 14px',
              display: 'inline-flex', alignItems: 'center', gap: 10,
              background: T.content, border: `1px solid ${T.border}`, borderRadius: 10,
              cursor: 'pointer', textAlign: 'left',
              color: T.text, fontSize: 13, fontFamily: 'inherit',
            }}>
              <span style={{ color: T.accent, display: 'flex', flexShrink: 0 }}>{s.icon}</span>
