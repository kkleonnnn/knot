...
日志写到 bi_agent/data/logs/bi_agent_{date}.log，rotate 每天，保留 7 天。
...
if not getattr(logger, "_bi_agent_configured", False):
    logger.remove()
    logger.configure(patcher=_patcher)
    logger.add(sys.stderr, level=_LEVEL, format=_FMT, enqueue=False)
    logger.add(
        str(_LOG_DIR / "bi_agent_{time:YYYY-MM-DD}.log"),
        level=_LEVEL,
        format=_FILE_FMT,
        rotation="00:00",
        retention="7 days",
    ...
    logger._bi_agent_configured = True
