"""bi_agent.config.settings — 集中加载 .env，导出 Settings 单例 + 常量。

设计原则（v0.3.0）：
  - 所有 os.getenv() 调用集中在本文件，业务代码不得直接读环境变量
  - 兼容期：保留模块级常量（MODELS/DEFAULT_MODEL/...）供老代码 `from bi_agent.config import X`
...
SQLITE_DB_PATH = os.getenv("SQLITE_DB_PATH", str(_project_root / "bi_agent" / "data" / "bi_agent.db"))
