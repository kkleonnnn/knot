...
```bash
cd /path/to/knot/data
# ⚠️ 必须连旁文件一起 —— `*.bak` 通配符**看不见**它们
ls -la *.bak *.bak-wal *.bak-shm 2>/dev/null
```

⚠️⚠️ **`*.bak` 通配符看不见 `-wal` / `-shm`** —— 而它们是 SQLite 的 WAL 旁文件，**任何一次只读打开
`.bak` 都会重新生成**（v0.9.12 实测：备份刚建完**没有**旁文件；只读打开一次后 `-shm`/`-wal` 出现）。
本机实测一次盘点：**10 个 `.bak` + 10 个旁文件，合计 19M**，而只按 `*.bak` 数会漏掉一半文件。

- **恢复时只需 `.bak` 主文件**（旁文件是读产物，可直接删）；
- **删除/归档时必须连旁文件一起**（否则残留半套，且下次 `ls *.bak` 看不到它们）。
