...
> **当前版本** v0.9.23 · 内测期（v0.6.1.4→0.6.5.6 升级 runbook 见 [docs/plans/v0.6.5.6-upgrade-from-v0.6.1.4-k8s.md](docs/plans/v0.6.5.6-upgrade-from-v0.6.1.4-k8s.md)；v0.6.5.x→v0.7.x 为纯内测迭代，无强制迁移步骤）
...
- **`replicas=1`**（⚠️ **v0.9.7 订正了这条的理由** —— 原文写「不同副本可能停在**不同租户的** catalog 上」，
  ...
  **多副本下真实的问题是「一个副本上的变更传不到其他副本」+「限流按副本各算一份」**，逐项实读如下：
  ...
  ⭐⭐ **这张表已废除（v0.9.23）—— 它此前在本文件与 `docs/plans/v0.9-lift-arc-remaining-plan.md`
  里各抄一份，而 R10' 逐条实读发现它 5 行里 4 行要改、另漏 3 项、多 1 项**（两份副本同时错）。

  ⇒ **单一来源 = `tests/test_process_state_registry.py` 的 `_REGISTRY`**（哨兵派生 + 双向相等：
  新增未登记即红、登记表留过期条目也红）。每条都写明「N 个副本各有一份时会发生什么」。
  查当前作业面请读那份登记表，**别读散文**。

  **本版（v0.9.23）已处置**：数据源引擎缓存（连接指纹进键 ⇒ 陈旧结构性消除 + 旧连接池 dispose）·
  启动期 audit 自动清理（原子认领 ⇒ 多副本不再并发 DELETE）。
  **⚠️ 仍未处置（已拆独立片）**：**限流桶每副本一份 ⇒ 有效限额 ×N**（登录爆破防护被削弱 N 倍）。

  ⚠️ **另两条本表从未包含、而实测存在的**：
  ① **首启并发建库会崩**：全新库 + 3 副本同时启动 ⇒ **2/3 崩在 `database is locked`**（本地盘实测，
     **与跨节点 NFS 无关**）—— 根因是 `.c4-migration.lock` 那把 flock **获取得比平台库初始化晚**，
     压根没盖住它。库已建好后 3 副本同起则正常。⇒ **首次部署仍须先单副本起、确认建库完成再扩容**。
  ② **RWO 卷下多副本必须同节点共置**（RWO = 同一时刻只能被一个节点挂载读写），
     否则第二个 pod 会卡在挂卷失败 —— 那与本节任何一项都无关，别误判。
