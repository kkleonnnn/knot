...
> **当前版本** v0.7.16 · 语义层第十七刀（派生指标模型：占比/人均 metric÷metric）：**首个非编译覆盖方向 — metric 模型层扩展**。占比/人均类**派生指标 = metric÷metric**（如客单价 = GMV÷订单数、ARPU = GMV÷DAU）走确定性编译 —— `SELECT (left 子查询) <op> [NULLIF](right 子查询) AS name`（FROM-less 0 JOIN，免疫基数坑）。lineage 列激活（v0.7.0 inert → 结构化 `{op,left,right}`，**0 schema ALTER**）；**注入安全**（op 白名单 divide/multiply/add/subtract + left/right 解析注册表原子 metric）+ **单层防循环**（left/right 须原子，嵌套派生回退）+ **divide→NULLIF 除零** + **per-dep time 注入**（「本月 arpu」各 dep 子查询各注入日期窗）；派生仅标量（维度/having/window → 回退）。admin 注册屏 +「派生定义」field。0 新 schema/路由/AuditAction；`KNOT_SEMANTIC_LAYER` 默认 off。<br>**上版** v0.7.15 自定义窗口 frame · v0.7.14 CTE 再聚合 · v0.7.13 抽 multi_base 重构 · v0.7.12 多 base+维度 · v0.7.11 多 base 标量 · v0.7.10 分区 top-N · v0.7.9 窗口函数 · v0.7.8 HAVING · v0.7.7 事件/规则/动作 · v0.7.6 标记采纳 · v0.7.5 版本历史 · v0.7.4 re-run · v0.7.3 审计/修正 · v0.7.2 跨对象 · v0.7.1 单对象 · v0.7.0 指标注册表。⚠️ OOS-1 死线 sustained
