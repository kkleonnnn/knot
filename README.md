...
> **当前版本** v0.6.0.10 · R-PA-5 内测期 · F-A/B/C/D 反馈观测治理基础设施完整（用户反馈 👍/👎 + 前端错误自动上报 + audit 自动清理 + 模型 catalog max_context + OR live sync）
...
- **加密**：Fernet 字段级（API key / DB 密码）+ KNOT_MASTER_KEY env fail-fast（v0.4.5+）+ **JWT_SECRET fail-fast**（v0.6.0.10 MUST-1）
