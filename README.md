...
> **当前版本** v0.7.15 · 语义层第十六刀（编译覆盖深化：自定义窗口 frame ROWS BETWEEN）：窗口函数支持自定义 `ROWS BETWEEN <start> AND <end>` frame —— 解锁**移动平均/滑动窗口**（如 7 日移动平均 `AVG(gmv) OVER (ORDER BY date ROWS BETWEEN 6 PRECEDING AND CURRENT ROW)`）。**frame 注入安全**（边界仅非负 int 或 `unbounded` 枚举，0 用户字符串裸拼）；**仅 sum/avg 聚合窗口**（ranking/lag/lead + frame → 回退）+ frame 需 ORDER BY；frame 是 window dict 子字段（canonical 递归排 → 语义相等等 canonical）；无 frame → v0.7.9 窗口 byte-equal；过 `is_cartesian` + `_is_safe_sql` 双安全门。**编译覆盖深化七刀齐**（HAVING/窗口/分区top-N/多base标量/多base维度/CTE再聚合/frame）。0 新 schema/路由/AuditAction；`KNOT_SEMANTIC_LAYER` 默认 off。<br>**上版** v0.7.14 CTE 再聚合 · v0.7.13 抽 multi_base 重构 · v0.7.12 多 base+维度 · v0.7.11 多 base 标量 · v0.7.10 分区 top-N · v0.7.9 窗口函数 · v0.7.8 HAVING · v0.7.7 事件/规则/动作 · v0.7.6 标记采纳 · v0.7.5 版本历史 · v0.7.4 re-run · v0.7.3 审计/修正 · v0.7.2 跨对象 · v0.7.1 单对象 · v0.7.0 指标注册表。⚠️ OOS-1 死线 sustained
