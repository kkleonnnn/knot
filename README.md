...
> **当前版本** v0.8.6 · ②b **仪表盘结构化拼板**：仪表盘从静态配置升级为**结构化 tile** —— 每板块一条自己的只读 SQL + 类型（KPI / 折线 / 圆盘 / 横条 / 表）+ per-tile 冻结快照，支持**拖拽排序 / 占列**（手写 DnD，无新依赖）。新 `bi_report_tiles` 子表：per-tile SQL 存前 `is_safe_sql` 只读校验 + 非 admin **逐 tile 脱敏**（R-BI-6）+ diff-by-id 同步（保未改 tile 快照 + id 归属校验防越权）。**整表原子刷新**（`run_in_executor` 卸载事件循环；②c 调度复用）。admin 结构化 builder 建/编，analyst 只读消费。走完整 v3 + 守护者 Stage 3 condition-accept（7 必修 + 4 裁定）。<br>**上版** v0.8.5 ②a **BI/ASK 双模**：共用同一 `<AppShell>`（侧栏壳 + topbar + 用户 footer 统一，只切侧栏中段 + 主区两 slot）+ inline 分段 `ModeToggle` + 共享 `RightPanel`（BI 数据分析 ∥ ASK 思考过程）；**宽表** admin 直写 SQL（`doris.is_safe_sql` 只读校验）+ 列注释表头 + **覆盖层公式**（零 eval `formula.js` 白名单求值器，双轮红队）+ 冻结快照 admin 控刷新；R-BI-1 **0 触 saved_reports/Chat**。<br>**更早** v0.8.4 导出注入中性化 · v0.8.0~.3 B6 语义激活门（待填 OHX corpus + eval-live 开 flag）· v0.7.x 语义层。⚠️ OOS-1 死线 sustained；②c 调度器 / ③ da-asst 真接顺延
