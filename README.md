...
> **当前版本** v0.8.14 · **UI 视觉升级**（设计侧三轮定稿；实施者只集成不改视觉）：**玻璃 chrome**（半透明 + backdrop-blur + 高光描边浮窗 + 页底环境色渐变）· **外观预设**（顶栏「外观」弹层：风格 雾面/极光 × 主题色 青/紫/翡翠/琥珀 × 浅/深/跟随系统，即时生效持久，OKLCH 同亮度换 hue 单色铁律守）· **自托管字体**（Inter + Noto Sans SC + JetBrains Mono，消 Windows 雅黑回退；字阶字重收敛 + tabular-nums）· **玻璃确认框**（替代原生 confirm）。<br>**上版** v0.8.13 **admin 批量运维**（few-shot/指标批量删除 · 指标 xlsx 上传 · 业务目录 JSON 模板）。<br>**更早** v0.8.12 **BI 设置 + 目录权限 RBAC**：BI 模式独立设置面板（数据源 / 用户 / API&模型 / 报表目录 / 目录权限）；新 `bi_permissions` = **按用户 × 目录/报表 × 4 权限**（定时 / 编辑 / 导出 / 分享，admin 恒全权，default-deny）—— 单租户内 user×resource（⚠️ OOS-1 死线 sustained，0 tenant_id）；da-asst 升为**第一类数据分析引擎**（独立模型槽 + BI 右栏只读报表解读 + 成本控制平面）。<br>**更早** v0.8.10~.11 **BI 仪表盘 12 列组件网格 builder**（6 类型 tile：KPI / 折线 / 圆盘 / 横条 / 表 / donut + 拖拽占列 + 冻结快照）+ 逐图视觉打磨。<br>**更早** v0.8.5~.9 **BI / ASK 双模**（共用 `<AppShell>` + 分段 ModeToggle + 共享 RightPanel；报表模式 = 仪表盘 tile / 多页 tabbed 报表 / 宽表 + 零 eval `formula.js` 覆盖层公式）· v0.7.x **5 层语义层 + LogicForm**（指标注册表 → NL→LogicForm→SQL 编译 → 混合路由；`KNOT_SEMANTIC_LAYER` flag 默认 off，待填真实 corpus + eval-live 达标后开）。
