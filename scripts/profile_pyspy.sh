#!/usr/bin/env bash
# scripts/profile_pyspy.sh — 用 py-spy 给运行中的 KNOT 抓火焰图 / 实时 top
#
# 用法：
#   1) 先启动 server： python3 -m uvicorn knot.main:app --port 8000
...
PID=$(pgrep -f "uvicorn knot.main:app" | head -1 || true)
if [[ -z "$PID" ]]; then
  echo "未找到 uvicorn knot.main:app 进程，先把 server 跑起来。" >&2
  exit 1
fi
