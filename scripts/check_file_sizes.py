...
BACKEND_ACK = {
    ...
    "knot/services/http_planner.py":       609,  # futures regex 下沉 catalog JIT v0.7.2 + v0.7.20 B（http_table_in_sql + failure_error_meta）+ v0.7.22 Layer A intent veto（_ANALYTICAL_INTENTS + early-return R-SL-162）565→579；v0.9.6 +Layer 0 owner 软降级门（8 行：谓词 + 日志 + early return；已压到下限 —— 详细论证在 executor.py 的硬边界注释里，不在此重复）579→588（headroom 2）；v0.9.7 +B-3 ② source_id 软降级门（3 行注释 + 7 行体）+ resolve_spec 契约重写（env 形态退役 + 「datasource 无条件覆盖」方向承重说明 —— 已压过一轮，剩下的全是「别改回去」的理由，删了就会被人改回去）588→607（headroom 2）
    ...
    "knot/services/engine_cache.py":       395,  # v0.9.1 失效器三拆（user/tenant 版 + tid 前缀键，346→361，MF3）；v0.8.24 +invalidate_all（340→346）；v0.9.23 R10'-A +34（3 站点连接指纹进键尾部 + `_put` 淘汰旧指纹条目并 dispose 旧连接池 —— 指纹计算已抽 `source_fingerprint.py` 独立模块，留在本文件的只有缓存机制本身）361→395
    ...
    "knot/main.py":                        375,  # v0.9.0 C2 +tenant 启动序 ❶-❺（platform bootstrap/逐租户 init_db）+ tenant middleware（249→306）+ C4 ❷.5 存量迁移 gate（+3）；app 工厂编排 inherent → ACK；v0.9.5 +平台密钥合规 WARN 钩子（→314）；v0.9.6 +起源租户未被服务 WARN 钩子（6 行 = 装配下限：decorator/def/docstring/2 行体）314→321（headroom 3）；v0.9.7 +起源租户 egress 走 env 回退 WARN 钩子（同一装配下限；`tenant_repo` / `_tenant_ctx` 已在顶部 import，故体内仅 1 行 import + 1 行调用）321→328（headroom 3）；v0.9.12 +静态明文巡检钩子 328→334（**装配下限 = 注释 1 + import 1 + 空行 1 + 调用 1 = 4 行**：逐租户循环与全部理由**已刻意搬进** `repositories/secret_at_rest.warn_all_tenants_at_startup`，main.py 只保留接线 —— 原先写在 main.py 里的版本要 +42 行，那不是 cap 该让路的理由）；v0.9.16 +私有 catalog 未挂载 WARN 接线（**装配下限 = 1 行调用**：函数体与全部理由都在 catalog_loaders）334→335（headroom 3）；v0.9.19 P-b 启动序**逐租户化**：新增 `_for_each_tenant` helper（约定 + 三条承重理由：为什么 `list_tenants()` 不是 `list_active_tenants()` · 为什么每租户各自 try · 为什么 `TenantContextError` 必须原样抛）+ audit purge 内联同形循环 338→370（headroom 5）。**helper 是净减重**：三处调用点从各 6 行压到各 1-3 行，理由集中在一处而不是抄三遍。
    "knot/repositories/tenancy_migration.py": 383,  # v0.9.0 C4 存量迁移：crash-safe + 并发-safe + 校验/fsync/resume-保全/denylist 防御纵深（守护者 Stage 4 两轮对抗加固）；v0.9.2 +db_dir 含容守卫（一处守 C4+uploads 两条写侧路径）；13 fn 单一内聚迁移关注点，拆分反损可读性 → ACK；v0.9.19 P-b 改**点名起源租户**（原 `resolve_single_tenant()` 是本函数第一行 ⇒ active≠1 时 boot 崩，且停用 t1+active t2 时会把锚点库迁进**错误目录**）370→378（headroom 5）。
