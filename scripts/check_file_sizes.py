...
BACKEND_ACK = {
    ...
    "knot/main.py":                        328,  # v0.9.0 C2 +tenant 启动序 ❶-❺（platform bootstrap/逐租户 init_db）+ tenant middleware（249→306）+ C4 ❷.5 存量迁移 gate（+3）；app 工厂编排 inherent → ACK；v0.9.5 +平台密钥合规 WARN 钩子（→314）；v0.9.6 +起源租户未被服务 WARN 钩子（6 行 = 装配下限：decorator/def/docstring/2 行体）314→321（headroom 3）；v0.9.7 +起源租户 egress 走 env 回退 WARN 钩子（同一装配下限；`tenant_repo` / `_tenant_ctx` 已在顶部 import，故体内仅 1 行 import + 1 行调用）321→328（headroom 3）
