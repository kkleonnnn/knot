...
- **共享 Foundation**（v0.5.6 + v0.5.7 落地）：
  - `Shared.jsx` — buildTheme(dark) 25 设计 token (含 dark 透传 = 26 runtime keys) + I 38 icons + iconBtn/pillBtn + CHART_COLORS 8 色 + LineChart/BarChart/PieChart/TypingDots + KnotMark/KnotWordmark/KnotLogo + **v0.6.2.3 整合 14 helper（SvgPath/KpiCard/StatusDot/NumChip/OverrideChip/SourceTag/FileIcon/LetterChip/DoneCheck/FormRow/FilterField/statCardStyle/statValueStyle/preStyle）→ 26 exports**
...
| ⏳ v0.6.2.4（Phase B 完整版段 3 · PATCH-2）| Shared drift 调和（PATCH-1 merge 后另起 · 全程守护强 active）| PeriodTab/TagChip/statLabelStyle/KV+DetailRow drift 参数化绝对 0 位移 + input/btn 族 lift-vs-fold + theadStyle 3 文件 + Avatar AST 三条件子集；3 雷区预锁（I.spark/flow 不进 dict / drift 0 位移 / SavedReports pillBtn 碰撞）|
