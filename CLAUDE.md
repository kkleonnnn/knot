...
1. **方案设计 (Stage 1)**：当前 Agent 产出执行手册 (`docs/plans/`)。
2. **资深审计 (Stage 2)**：提交给资深架构师（User）和辅助 AI 获取 Redline。
3. **溯源复审 (Stage 3)**：将方案喂给上一代 Agent 环境进行逻辑一致性校验。
4. **终审锁定 (Stage 4)**：整合所有意见，由当前 Agent 完成最终执行手册并确认。
