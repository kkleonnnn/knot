...
| ⏳ v0.6.2.6（Phase B 完整版段 4 · A1 **并发半**）| Connection Context 隔离三层 + query per-user 解析 | ContextVar `_active_catalog_ctx`（query_steps 入口捕获 active_catalog_id）+ SQL 执行前 assert + **catalog.context_violation audit 39→40** + query.py Extract Method 瘦身（12 行余量）+ race 100× 守护 + Q4 copy_context 预防性（若引入 create_task/gather/threadpool）— 待 Stage 1 草案 |
