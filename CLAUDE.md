...
| ⏳ v0.5.17+ | (C5+) 剩余 10+ 屏 | admin/users 独立 PATCH（含 #ff7a3a 渐变残留偿还）+ 8 admin tabs + database / knowledge / catalog / sql-lab / settings / conversations 5 业务屏；执行模板由 v0.5.7~v0.5.16 十 pilot 确立；视觉复刻通用约束由 v0.5.8 § Visual Replication Protocol + 字面分流体系三条（R-227.5 / R-227.5.1 / R-302.5）+ R-286 hex 全面禁止 + **R-386 brandSoft 8% 全站三处闭环字面** + **I.db 资产复用先例（Shared 优先）** + v0.5.14 红线撤回首例设计先例锁定；v0.6 路线图新增 **ResultBlock 子组件拆分**承诺（R-341 锁定 — 6 候选） |
