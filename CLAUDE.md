...
| ⏳ v0.5.12+ | (C5+) 剩余 14 屏 | thinking / favorites / 9 admin tabs / database / knowledge / catalog / sql-lab / settings / conversations；执行模板由 v0.5.7 Login + v0.5.9 Shell + v0.5.10 Home + v0.5.11 Composer 四 pilot 确立；视觉复刻通用约束由 v0.5.8 § Visual Replication Protocol 锁定；视觉自动跟随设计模式（R-251 + R-264）首次确立后可应用于其他组件复用场景 |
