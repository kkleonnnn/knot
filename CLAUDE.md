...
| ⏳ v0.5.14+ | (C5+) 剩余 12 屏 + ResultBlock 视觉重构 | favorites / 9 admin tabs / database / knowledge / catalog / sql-lab / settings / conversations + ResultBlock 视觉大重构（table card / chart card / insight inset / agent_costs chip）；执行模板由 v0.5.7~v0.5.13 七 pilot 确立；视觉复刻通用约束由 v0.5.8 § Visual Replication Protocol + 字面分流体系三条（R-227.5 / R-227.5.1 / R-302.5）+ R-286 hex 全面禁止锁定 |
