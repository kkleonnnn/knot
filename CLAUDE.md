...
⭐⭐ **同族的可操作化形式（v0.9.19 立 · kk 拍板折入）—— 它比上面那句更好用，因为它给出了「去问什么」**：
> **凡是「门校验 X，而系统随后用 X 去做某事」的地方，都要实测
> 「**门看到的 X**」与「**真正被用的 X**」是不是同一个值。**

⚠️ 这里的失效**不需要任何人写错代码** —— 两个值可以在**语言/库层面**就不是一回事，
而代码读起来完全正常。⇒ 靠读 diff、靠 code review **都发现不了**，只能实测。

| 门校验的 | 真正被用的 | 实证 |
|---|---|---|
| bundle 里**写着**的版本字面 | `index.html` **真正引用**的那个 chunk | v0.9.10（连漏 3 片无人察觉） |
| `schema.sql` **声明**的列 | 库里**真的建出**的列 | v0.9.12（文本解析实测 3 处假红） |
| ⭐ `url.host`（allowlist 会校验它） | `dialect.create_connect_args(url)` 的 `host`（**真正连过去**） | **v0.9.19**：`db_database` 填 `db1?host=evil…#` ⇒ 两者分别是 `allowed.corp` 与 `evil.example.com` ⇒ **任何校验 host 的门都是装饰品** |

⇒ **修法优先级**：让两者**结构上不可能不同**（v0.9.19 用 `URL.create()` 按组件构造）
**优于**在门里过滤某个 payload —— 后者消灭一个 payload，前者消灭**这一类**。
⚠️ 且**别在中间放字符串**：`URL` → 字符串 → `make_url()` 解析回来，**那一步就是注入重现的地方**。
...
1. **fail-open 面**：新判据写错会 fail-open 还是 fail-closed？
   - ⭐ **CLI / 工具形态的 fail-open（v0.9.15 折入 · kk 拍板）：破坏性工具不得有默认目标。**
     缺目标即**拒绝执行**（非 0 退出 + **零写入**）。
     ⚠️ **「可选 + 回退唯一/默认对象」在对象集合从 1 变成 N 的那一刻起就是 fail-open** ——
     它在只有一个对象的年代无处可错，所以看起来一直是对的。
     **实证（同一片三个实例）**：`reset_admin_password` 无参数解析 ⇒ flag 被**静默吞掉**
     ⇒ 重置了**起源租户**（部署方自己）的 admin 口令并打印「已重置」，**运维看不出异常**
     （这不是推理 —— 真的发生了）· `migrate_encrypt_v045`（重写凭据列）与 `purge_audit_log`
     （删审计行）同形，由本片「新租户恒 `suspended`」引发**回退恒中起源租户**。
     ⇒ 只读预览（`--dry-run`）可保留回退，但**必须打印它解析到了谁**；
     且**改了工具就要扫两侧**（见 #8）—— 活文档里的调用范例照抄必被拒是本片实测漏过的。
     守护：`tests/scripts/test_destructive_cli_requires_target.py`（行为 + AST + 分类 + 文档四层）。
     ⭐⭐ **可操作的推论（v0.9.15 Stage 4 守护者背书 · kk 2026-08-06 采纳）**：
     **任何把「唯一」变成「多个」的片，都必须重扫所有依赖「唯一」的回退。**
     实证：v0.9.15 让新租户恒 `suspended` 这**一个前提变更**，**同时**让三个脚本的回退变成
     fail-open —— **不是三次疏忽，是一次前提变更没有被扫**。
     ⇒ 判据形式：改动前问「**本片让哪个集合从 1 变成了 N**」，然后 grep 所有依赖「那个唯一」的回退点。
   - ⭐⭐ **同族第二条推论（v0.9.18 P-a 立 · kk 背书合并）——「把 X 域化 / 给 X 加门」时清扫面怎么定**：
     > **先列出「所有需要被 X 管住的行为」，再看哪些没被管** ——
     ...
     **三种伪装，各有实证**：
     | 伪装 | 实证 |
     |---|---|
     | **被声明为无关的** | `KNOT_WEBHOOK_ALLOWED_HOSTS` 的 docstring 自称「**独立** egress allowlist、**≠** 数据源那个」⇒ v0.9.7 修 `url_allowlist` 时它**结构上被排除在扫描面外**（kk 自陈：他做了那片的 Stage 3/4） |
     | **形态不同的** | 按「dict/set 字面量」扫模块级可变状态得 **31 处**；按**行为**（跨请求持有可变状态，含任意构造调用）扫得 **75 处** ⇒ **漏 44 处**，而要找的 `TTLCache(...)` 正藏在里面。`@lru_cache` 更狠 —— 它**连赋值语句都没有** |
     | ⭐ **压根不在的** | `api/admin/datasources.py` 的 SSRF 守卫**函数体第一行**是 `if db_type != "http": return` ⇒ **SQL 数据源那条出网路径零 allowlist** ⇒ 按「找名单」的扫描面**结构上找不到它**（三个独立 lens 里只有一个找到） |

     ⚠️ **前两类是「同类物被伪装」，第三类是「同类物不存在」** —— 后者只能靠
     「**先列行为、再看谁没被管**」找到，这就是判据必须从「扫实例」改成「扫应管行为」的原因。
...
3. ⭐ **「那一行」族**（v0.9.8 Stage 4 §II-2 合并 —— 原 #3 门 / #4 消息 / 新形状「记录」同族）：
   **门 / 消息 / 记录，都要在「事情真的发生的那一行」。**
   ...
   - ⭐ **层对了，但入口不止一个**（v0.9.15 Stage 4 折入 —— 守护者自陈他也漏了这条）：
     门装对了那一行**之后**，还要问「**这个能力有几个入口？我的门在每一个前面吗？**」
     实证：`db_dir` 唯一索引有**两个创建点**（`platform_schema.sql` 的 `CREATE` 与迁移里的 `CREATE`），
     而 `init_platform_db()` **先跑 schema** ⇒ 存量库带重值时 schema 那行**先炸**、抛裸 `IntegrityError`，
     精心写的**点名预检压根没被执行到**。
     ⚠️ **与前三条的差别**：那三条是**门装错了层**；这条是**层对了而入口不止一个**。
     ⚠️⚠️ **它对读 diff 的人也是隐形的** —— 两个 `CREATE UNIQUE INDEX` 都在 diff 里、看得见；
     **看不见的是执行顺序**。守护者复核时的原话：「我只需问一句『哪个先跑』。我没问。」
...
2. ⭐ **每条「已守护」的声称都有一次真跑过的 revert-to-bad**，且**把失败信息的原文贴进记录** ——
   ...
   **⑥ 是探针打在了另一个系统上**，而那个系统的反应恰好和我预期的一样。
   ...
   > ⭐⭐ **⑥ 的第二形态（v0.9.21 立 · 守护者提出，执行者认同不新增第 12 条）**：
   > **端到端 oracle 的「可达性」本身是环境变量的函数。**
   > **实证**：出网测里设了 `HTTP_PROXY` 之后，**目标监听器 0 收到**、代理监听器收到
   > ⇒ 「零发送」这条断言**恒真** —— 而它看起来像证据。
   > ⚠️ 它与 #7「顶班」不同（那是被测对象连不通），也与 ⑥ 原形态不同
   > （那是**探针打在另一个系统上**；这里探针**打对了**，是**通路被改道**）。
   > ⇒ **机械形式**：**否定断言必须与同一测内的可达性正对照绑定** ——
   > 先证「这个监听器这会儿真收得到」，再断「它没收到」。
   > 这与本仓既有的「`caplog` 反向断言须先证 `caplog` 非空」是**同一条规则的网络版**。
   > 载体：`tests/adapters/test_url_canon_single_parser.py` 的 `listener` fixture（清空 6 个代理 env）
   > + 每条否定断言前的 `/reachable` 正对照。
...
> - ✅ **webhook 出网租户域化 + lark token 缓存键 = v0.9.18（P-a）已闭合** ——
>   ⚠️ **此前两项都不在本清单里**（「清单必须派生」的**第四个数据点**）：
>   `adapters/notification/webhook.py` 的 `KNOT_WEBHOOK_ALLOWED_HOSTS` 与 v0.9.7 修掉的
>   `url_allowlist` **完全同形**，却因 docstring 自称「独立、≠ 数据源那个」而逃过那次清扫；
>   `lark._token_cache` 的键不含租户**也不含 secret**（命中路径上 `app_secret` 一次未读
>   ⇒ **命中缓存时调用方没有证明过任何东西**）。
>   ⚠️ **诚实边界**：webhook 的 allowlist **仍只管第一跳**（`requests.post` 未禁重定向）⇒
>   **不得声称「webhook 出网已受控」**，只能声称「allowlist 已按租户区分」。重定向禁令在 **P-a'**。
> - ▶ **P-a'（下一片）**：4 个出网点禁重定向 + **SQL 数据源出网无 allowlist** 的口径
>   （kk 2026-08-07 裁定：**host 由部署方控，不接受租户自填**）。
