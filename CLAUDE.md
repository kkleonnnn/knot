...
| ⏳ v0.5.15+ | (C5+) 剩余 12 屏 | favorites / 9 admin tabs / database / knowledge / catalog / sql-lab / settings / conversations；执行模板由 v0.5.7~v0.5.14 八 pilot 确立；视觉复刻通用约束由 v0.5.8 § Visual Replication Protocol + 字面分流体系三条（R-227.5 / R-227.5.1 / R-302.5）+ R-286 hex 全面禁止 + **v0.5.14 红线撤回首例设计先例**（红线服从视觉真理）锁定；v0.6 路线图新增 **ResultBlock 子组件拆分**承诺（R-341 锁定 — 6 候选 MetricCard/TableContainer/InsightCard/BudgetBanner/ErrorBanner/TokenMeter） |
