...
- **✅ v0.7.6** LogicForm「标记采纳此版本」（append-only 恢复 · 资深 announce 拍语义）— semantic_query_audit +`restored_from_audit_id`（schema + 幂等 ALTER）+ `POST /logicform-audit/{id}/restore`（源版本忠实字节复制新建修正行 is_corrected=1 + restored_from + logicform.rollback audit；append-only 0 删/0 改历史 + 0 重编译 + catalog 继承）+ LogicFormHistory「标记采纳此版本」按钮（R-SL-61.3 严禁「恢复」+ inline note）+「采纳自 #N」徽标；R-SL-57~64；**⭐ R-SL-61 三层诚实栈**（守护者 Stage 3：恢复**不回流查询路径**+不改 metric 口径 → 治理留痕非改查询行为；61.1 运行时证明 + 61.2 文档 + 61.3 按钮诚实「标记采纳」非「恢复」；/restore≈/correct 便利封装）；AuditAction 46→47；详 [docs/plans/v0.7.6-logicform-rollback.md](docs/plans/v0.7.6-logicform-rollback.md)
- **✅ v0.7.7** 事件/规则/动作三层 vertical slice（资深拍一次连做上 3 层端到端可测）— `semantic_monitors`（事件+规则+动作合一 D2）+ monitor_trigger_audit 留痕 + monitor_repo + `monitor_eval`（compile_logicform metric-only 取标量 + 阈值/环比，0 LLM）+ `WebhookNotificationAdapter`（兑现预留 Protocol）+ `POST /monitors/check-now`（手动「立即检查」D1）+ AdminMonitors 屏；AuditAction 47→51 / admin 路由 40→46；R-SL-65~77；**克制三剪**（手动触发无调度 / 通知非写回 read-only / 阈值非 LLM 归因）；**⭐ 守护者 Stage 3 三纠正**：R-SL-69 独立 `KNOT_WEBHOOK_ALLOWED_HOSTS`（不混数据源读取 allowlist）+ R-SL-77 fire 受 `KNOT_SEMANTIC_LAYER`（零生产风险不变量）+ R-SL-75 re-fire 诚实；详 [docs/plans/v0.7.7-event-rule-action-slice.md](docs/plans/v0.7.7-event-rule-action-slice.md)
- **✅ v0.7.8** 对象层编译覆盖深化：HAVING（聚合后过滤 · 资深拍 ② 不碰新基础设施 kk 边做边测）— LogicForm +`having`（canonical 空省略键末位 R-SL-81 存量 byte-equal）+ compiler `_having_clause` 单/跨对象两路（子句序 GROUP BY→HAVING→LIMIT）+ parser prompt 教 HAVING；R-SL-78~83；**⭐ 守护者 Stage 3 R-SL-80 多对象 alias-based HAVING**（强制引 metric alias，严禁 raw o.col/裸 caliber —— 多对象 o. 不重写会断；执行者坐实 _build_multi_object_sql lf.filters 原始不重写非对称）；HAVING 注入 _is_safe_sql DQL-only 收口；having 空 byte-equal；**0 新 schema/路由/AuditAction**；详 [docs/plans/v0.7.8-compile-coverage-having.md](docs/plans/v0.7.8-compile-coverage-having.md)
- **✅ v0.7.9** 对象层编译覆盖深化：窗口函数（排名/同环比/累计 · 资深 announce 窗口）— LogicForm +`window`（canonical 空省略键末位 R-SL-88）+ compiler `_WINDOW_FUNCS` 白名单 + `_window_col` + `_finalize` 两层（inner=sql+having 不调 _order_limit / 外层 OVER 引 alias + 整个 _order_limit / 无 window 单层 byte-equal）+ parser prompt 教 window；R-SL-84~90；**⭐ R-137 框法纠正（守护者 Stage 3 F1）**：单层窗口+GROUP BY 合法（OVER 引聚合表达式），**两层为 alias 干净而选非单层不可能**（我 + Stage 2 共写假前提已纠）+ **F3 dense_rank 非 rank_dense**（Stage 2 笔误，真实 SQL 函数名 + 每 func 测试）；**0 新 schema/路由/AuditAction**；compiler ACK 320；详 [docs/plans/v0.7.9-compile-coverage-window.md](docs/plans/v0.7.9-compile-coverage-window.md)
- **✅ v0.7.10** 对象层编译覆盖深化：分区 top-N（窗口结果过滤 · 资深拍 ② 编译覆盖续）— 承 v0.7.9 窗口列 → 在排名列上过滤「前 N」；LogicForm +`qualify`（QUALIFY 语义 · canonical 空省略键末位 R-SL-91）+ compiler `_finalize` **递进 byte-equal 三层**（无 window 单层 / window 无 qualify 两层 byte-equal / window+qualify 三层 `SELECT * FROM (<两层>) win WHERE <qualify>`）；QUALIFY 语义用**外层 WHERE**（不赌 Doris QUALIFY；`Subquery` ∈ `_is_safe_sql` allowed_roots）+ `_order_limit` 只最外层一次（R-SL-92）+ parser prompt 教 qualify；R-SL-91~97；**⭐ 守护者 Stage 3 纠 Stage 2 错框法**：qualify alias 正确性守护在 **parser 死线教学（门 B），非 `_is_safe_sql`**（门 A 只 DQL-only 收口 — 两个门别混；raw o.col qualify 过 `_is_safe_sql` 但 DB 报未知列）+ R-SL-94 悬空 qualify→CompileError（严禁静默丢弃返全量）；**0 新 schema/路由/AuditAction**；compiler 309 ≤320 ACK；详 [docs/plans/v0.7.10-compile-coverage-partition-topn.md](docs/plans/v0.7.10-compile-coverage-partition-topn.md)
- **✅ v0.7.11** 对象层编译覆盖深化：多 base 标量聚合（资深拍 编译覆盖续 · 最高风险刀 v0.7.2 基数承重坑区域）— 让标量多 base（「本月 GMV 和 DAU」gmv@orders+dau@users）走确定性编译而非 R-SL-31 raise；compiler `_resolve_single_object`→`_metric_bases` + `_build_sql` 多 base 分支 + `_build_multi_base_sql` **标量子查询入 SELECT**（每 metric `(SELECT caliber FROM physical o WHERE filters[AND time]) AS name` 保序 · 外层 FROM-less **0 JOIN**）；parser `_validate_hit` 放宽（标量多 base）+ prompt 铁律放宽；R-SL-98~104；**⭐ R-SL-100 基数安全 BY CONSTRUCTION**（0 JOIN 各子查询 1 行 → 免疫 v0.7.2「JOIN 后聚合」膨胀坑，不需 joingraph）；**⭐ 守护者 Stage 3 两纠**：① R-SL-101 全矩阵 raise-guard（dims/having/window/qualify/filters 任一→raise CompileError；纠 Stage 2 `assert not dimensions` 漏项 + assert 机制错）② 抽 module 带循环 import 雷（CompileError 5+ 外部 `compiler.CompileError` 引用）→ 资深拍 **C 留 compiler + ACK bump 320→355**（0 循环 import）；过 is_cartesian + _is_safe_sql 双安全门；**0 新 schema/路由/AuditAction**；详 [docs/plans/v0.7.11-compile-coverage-multi-base.md](docs/plans/v0.7.11-compile-coverage-multi-base.md)
- **✅ v0.7.12** 对象层编译覆盖深化：多 base + 维度「聚合后 JOIN」（资深拍 编译覆盖续 · 承 v0.7.11 标量多 base 加维度）— 「各城市 GMV 和 DAU」走确定性编译；**JOIN 类型抉择资深拍 Option U 维度并集驱动**（UNION + LEFT JOIN 各聚合 → 对称不丢 + Doris-proven，0 FULL OUTER 避 Doris 未知）；compiler `_build_multi_base_sql` 拆分发器 + `_build_multi_base_scalar_sql`（v0.7.11 平移 byte-equal）+ `_build_multi_base_dimensional_sql`（`(各 metric DISTINCT dims+own filter UNION) dim LEFT JOIN 各 agg ON dim.d=t{i}.d`）；parser `_validate_hit` 去多 base+维度拒（compiler 共同维度 authoritative）+ prompt 教共同维度；R-SL-105~111；**⭐ R-SL-108 基数 BY CONSTRUCTION**（聚合后 JOIN 1:1 不膨胀，非 v0.7.2 坑）；**⭐ 守护者 Stage 3 两纠**：① **per-metric agg bug**（Stage 1 per-base 合用 `names[0]` filters → 同 base 不同 filter metric 静默算错；两 stage 都漏，守护者深挖发现 → per-metric 各自 filter，与 scalar 一致）② NULL-dim `=`+注释写明 OSS-空维度须 `<=>`（资深拍 `=`，DWD-clean 数据域）；过 is_cartesian + _is_safe_sql 双安全门；**0 新 schema/路由/AuditAction**；compiler 394 ≤400 ACK（逼近 → 下刀抽 multi_base.py 纯重构）；详 [docs/plans/v0.7.12-compile-coverage-multi-base-dimensional.md](docs/plans/v0.7.12-compile-coverage-multi-base-dimensional.md)
- v0.7.x+ 编译覆盖续（多 base+维度其他 JOIN 变体/自定义 frame/CTE 各独立刀）+ **抽 multi_base.py 纯重构**（compiler 逼近 400）+ 定时主动评估（调度器 + fired-state 去重）+ 自动执行动作（写回 · 高风险）+ 回滚改变查询行为（修正回流 · Stage 0 预研）
